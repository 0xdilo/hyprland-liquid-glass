...
void CLayerGlassPassElement::draw(const CRegion&) {
    ...
    if (!monitor || !g_pHyprOpenGL->m_renderData.currentFB)
        return;
    ...
    LiquidGlass::GlassRenderer::sampleBackground(m_data.state->sampleFramebuffer, *source, transformedBox, m_data.state->samplePaddingRatio, downscale);
    ...
    LiquidGlass::GlassRenderer::blurBackground(m_data.state->sampleFramebuffer, blurRadius, blurIterations, source->getFBID(), viewportWidth, viewportHeight);
    ...
    LiquidGlass::GlassRenderer::applyGlassEffect(m_data.state->sampleFramebuffer, *source, layerBox, transformedBox, alpha, cornerRadius, 3.0F,
                                                 m_data.state->samplePaddingRatio, m_data.maskTexture);
}
